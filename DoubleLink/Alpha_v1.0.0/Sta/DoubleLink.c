...
* @return		返回值为0 为正确排序
*/
int CallBackSequence(DLNode *List, DLNode *p_one, DLNode *p_two, int flag)
{
	DLNode *temp=NULL;
	void *temp_data;
...
*/
DLNode *SequenceList(DLNode *List, int flag, int (*CallBackSequence)(DLNode *,DLNode *, DLNode *, int flag))
{
	DLNode *p_one;
	DLNode *p_two;
	...
	p_one = List->next;
	p_two = List->next;
	CallBackSequence( List, p_one, p_two, flag );
	printf("Done\n");
}
