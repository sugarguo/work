...
void InsertList(DLNode *List, void *data, int (*CallBackCmp)(const void *, const void *))
{
	...
	else
	{
		...
		while(p1 != NULL)
		{
			if(p1->next == NULL)
			{
				p1->next = (NetNode *)data;
				(NetNode *)(data)->front = p1;
				break;
			}
			if(CallBackCmp(p1,data) > 0)
			{
				if(p1->front == NULL)
				{
					(NetNode *)(p->data) = (NetNode *)data;
					(NetNode *)(data)->next = p1;
					p1->front = (NetNode *)data;
				}
				else
				{
					(NetNode *)(data)->front = p1->front;
					p1->front->next = (NetNode *)data;
 ...
 * @todo      null
 */
void DeleteList(DLNode *List, void *data, int (*CallBackCmp)(const void *, const void *), void (*CallBackDelete)(DLNode *))
{
	DLNode *p = NULL;
	p = (DLNode *)malloc(sizeof(DLNode));
